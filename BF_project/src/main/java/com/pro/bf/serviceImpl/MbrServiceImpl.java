...
public class MbrServiceImpl implements MbrService {
	...
	@Override
	public ArrayList<MbrVO> memberList(int page, String search,
			String selectCombo) throws SQLException { // 회원 리스트
		...
		startRow = (page - 1) * counts; // 초기값 0 --> 현재 몇페이지인지에 대해 제일 먼저 시작하는
										// 공지사항 글
		...
		if (selectCombo.equals("ID")) {
			memberList = mbrDao.memberListforId(search, startRow, counts); // ID로
																			// 검색한다.
		} else if (selectCombo.equals("PHONE")) {
			memberList = mbrDao.memberListforPhone(search, startRow, counts); // 핸드폰번호로
																				// 검색한다.
		} else if (selectCombo.equals("NAME")) {
			memberList = mbrDao.memberList(search, startRow, counts); // 기본으로
																		// Name으로
																		// 검색한다.
		}
		return memberList;
	...
	// 회원 전체 개수<검색어로>
	public int totalRecord(String search, String selectCombo)
			throws SQLException {
	...
	@Override
	public String pageNumber(int page, String search, String currentPage,
			String selectCombo) throws SQLException {
		String str = "";
		...
		if (start_page > view_rows) {
			str += "<a href='" + currentPage + "?page=1&search=" + search
					+ "'>&lt;&lt;</a>&nbsp;&nbsp;";
			str += "<a href='" + currentPage + "?page=" + (start_page - 1)
					+ "&search=" + search + "'>&lt;</a>&nbsp;&nbsp;";
		}
		...
		for (int i = start_page; i <= end_page; i++) {
			if (i == page) {
				...
				str += "<a href='" + currentPage + "?page=" + i + "&search="
						+ search + "'>[" + i + "]</a>&nbsp;&nbsp;";
			}
		}

		if (page_count > end_page) {
			str += "<a href='" + currentPage + "?page=" + (end_page + 1)
					+ "&search=" + search + "'>&gt;</a>&nbsp;&nbsp;";
			str += "<a href='" + currentPage + "?page=" + page_count
					+ "&search=" + search + "'>&gt;&gt; </a>&nbsp;&nbsp;";
		}
	...
	@Override
	public void reuseMember(String memberId) throws SQLException { // 회원 다시
																	// 사용가능으로
																	// 만들기
	...
	@Override
	public ArrayList<MbrVO> memberListForUseyn(String choiceUseYNT)
			throws SQLException { // UseYN 으로 회원리스트 받아오기
		ArrayList<MbrVO> memberListUse = mbrDao
				.memberListForUseyn(choiceUseYNT);
		return memberListUse;
	}

	@Override
	public MbrVO MemberVoSearch(String memberId) throws SQLException { // 아이디로
																		// 회원정보
																		// 가져오기
																		// |
																		// 상세보기
	...
	@Override
	public void memberUpdate(MbrVO mbrvo) throws SQLException { // 관리자가 회원정보
																// 수정하기
		// 관리자가 수정할 수 있는 회원의 정보는 pw | name | phone | email | address 5개만 | id는
		// key값으로 사용
