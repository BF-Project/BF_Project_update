...
public class CommunityServiceImpl implements CommunityService{
	...
	public ArrayList<CommunityVO> communityList(int page, String search, String selectCombo) throws SQLException {
		...
		if(selectCombo.equals("first")){ // 초기값
			communityList = cmmtDaoImpl.communityListForFirst(search, startRow, counts2); // Title로 검색한다.
		}else if(selectCombo.equals("ID")){
			communityList = cmmtDaoImpl.communityListForIDIDID(search, startRow, counts2); // mbr_id로 검색한다.
		}
	...
	public int totalRecord(String search, String selectCombo) throws SQLException{
		...
		if(selectCombo.equals("first")){ // first ==> 초기값
			total_pages = cmmtDaoImpl.totalCommunityList(search); // 제목으로 찾으나..order by는 num으로 
		}else if(selectCombo.equals("ID")){
			total_pages = cmmtDaoImpl.communityListforID(search);
		}
		return total_pages;
	}
