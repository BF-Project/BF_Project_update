...
<head>
	...
	<style>
		.ggggTwo, .navbar-inverse .navbar-nav>.active>a:focus,
		.navbar-inverse .navbar-nav>.active>a:hover {
		color: #fff;
		background-color: #080808;
	}
	...
	<script>
		$(function(){
			$('input[type="checkbox"][name="checkCommunitySelectAll"]').click(function(){
				// click 이벤트가 발생했는지 체크
				if($(this).prop('checked')){ // 체크시
					$('input[type="checkbox"][name="checkCmmt"]').prop('checked', true);
				}else{
					$('input[type="checkbox"][name="checkCmmt"]').prop('checked', false);
				}
			});
		})
	</script>
	...
	<script>
		function selectDelete(){
			var checkCmmtNumValue = new Array();
			$('input[type="checkbox"][name="checkCmmt"]:checked').each(function (index){  
				checkCmmtNumValue[0] += $(this).val() + ",";  
		    });
			$.ajax({
				url:"<%=request.getContextPath()%>/admin/cmmtSelectDelete",
				type:"post",
				dataType:"text",
				data:({
					checkCmmtNumValue : checkCmmtNumValue[0]
				}),
		...
		function detailCmmt(cmmtNum){
			// 필요한게...페이지번호랑, 콤보박스 뭐 선택했는지가 필요하고
			location.href='<%=request.getContextPath()%>/admin/communityDetail?cmmtNum='+cmmtNum+'&page=${page}&comboSelectCommunity=${param.comboSelectCommunity}';
		}
	</script>
</head>
<body>
	<c:remove var="cmmtcmtListSize" scope="session"/> <!-- communityDetail에서 session이 지워지지 않았을 경우를 대비해 하나 더 적어놓음 -->
			...
			<div class="row">
				...
				<div class="table-responsive">
					...
					<thead>
						<tr style="font-size: 20px">
							<th style="text-align:center; width:50px">
								<span class="checksall">
									<input type="checkbox" id=selectAll name=checkCommunitySelectAll>
						...
						<c:choose>
							<c:when test="${CommunityListSize <=0 }">
								<tbody>
									<tr>
										<td colspan="6" align="center">커뮤니티 게시판 목록이 없습니다.</td>
									</tr>
								</tbody>
							</c:when>
							<c:otherwise>
								<tbody>
									<c:forEach items="${communityList}" var="cmmtVO" varStatus="count">
										<tr id="hoverId">
											<td>
												<span class="checks">
													<input type="checkbox" id=${count.index} name=checkCmmt value="${cmmtVO.cmmt_num}">
													<label for=${count.index}></label>
												</span>
											</td>
											<td onclick="detailCmmt('${cmmtVO.cmmt_num}')">${cmmtVO.cmmt_num}</td>
											<td onclick="detailCmmt('${cmmtVO.cmmt_num}')">${cmmtVO.cmmt_title}</td>
											<td onclick="detailCmmt('${cmmtVO.cmmt_num}')">${cmmtVO.mbr_id}</td>
											<td onclick="detailCmmt('${cmmtVO.cmmt_num}')">${cmmtVO.cmmt_date}</td>
											<td onclick="detailCmmt('${cmmtVO.cmmt_num}')">${cmmtVO.cmmt_cnt}</td>
										</tr>
									</c:forEach>
								</tbody>
							</c:otherwise>
					...
					<c:if test="${empty paging}">
						...
						<div id="search-form">
							<form method="get" action="CommunityList">
								<c:if test="${param.comboSelectCommunity eq 'first' or param.comboSelectCommunity eq '' or param.comboSelectCommunity eq null}">
									<select name="comboSelectCommunity" style="height: 30px">
											<option value="first" selected>제목</option>
											<option value="ID">아이디</option>
									</select>
								</c:if>
								<c:if test="${param.comboSelectCommunity eq 'ID'}">
									<select name="comboSelectCommunity" style="height: 30px">
