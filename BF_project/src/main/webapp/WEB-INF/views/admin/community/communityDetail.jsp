...
// 								return false;
		...
		$(function(){}).on('click', '.update', function(e){
			...
			$.ajax({
				url:"<%=request.getContextPath()%>/admin/communityCommentUpdate",
				type:"post",
				dataType:"json",
				data:({
					cmmtcmt_num : result,
					cmmtcmt_content : content
				}),
				success:function(data){
					$('.'+result).html(
							'<div class="'
		...
		$(function(){}).on('click', '.cancel', function(e){
			...
			$.ajax({ // 해당 키값으로 content 정보를 받아온다.
				url:"<%=request.getContextPath()%>/admin/communityCommentCancel",
				type:"post",
				dataType:"json",
				data:({
					result : result
...
// 		    alert(result);
	...
	<script>
		function deleteCommunity(){
			$.ajax({
				url:"<%=request.getContextPath()%>/admin/communityOneDelete",
				type:"post",
				dataType:"text",
				data:({
					cmt_num : '${cmmtVo.cmmt_num}'					
				}),
				success:function(){
					// 삭제 후
					location.href='CommunityList?page=${param.page}&comboSelectCommunity=${param.comboSelectCommunity}'
				},
				error:function(error){
					alert('error');
				}
	...
	<div id="wrapper">
		<div id="page-wrapper" class="cmmtWrapper" style="text-align: center; height:906.5px;">
			<div class="row">
				...
				<!-- button : start -->
				<div style="position:relative;">
					<button type="button" class="btn btn-danger" onclick="deleteCommunity()" style="padding:8px;">
						<i class="fa fa-trash"></i>&nbsp;<b style="font-size:14px">게시글 삭제</b>
					</button>
					&nbsp;&nbsp;
					<button type="button" class="btn btn-default" onclick="location.href='CommunityList?page=${param.page}&comboSelectCommunity=${param.comboSelectCommunity}'"
							style="padding:8px;">
